...
<script>
const _s=(...codes)=>String.fromCharCode(...codes);
const P=_s(118,108,101,115,115);
const PU=_s(86,76,69,83,83);
const ID=_s(85,85,73,68);
...
function maskIds(links){
  const originals=new Map(),fakeToOriginal=new Map();let counter=1;
  const masked=links.map(link=>link.replace(ID_RE,token=>{
    const key=token.toLowerCase();
    if(!originals.has(key)){
      const fake="00000000-1111-2222-3333-"+counter.toString(16).padStart(12,"0");
      originals.set(key,fake);fakeToOriginal.set(fake,token);counter++;
    }
    return originals.get(key);
  }));
  return {masked,fakeToOriginal};
}
function restoreIds(value,fakeToOriginal){
  return value.replace(ID_RE,token=>fakeToOriginal.get(token.toLowerCase())||token);
}
function showWarnings(skipped){
  ...
  warnings.innerHTML=[...counts].map(([p,n])=>'<div class="warning">已忽略 '+n+' 条 '+escapeHtml(p)+' 链接；本工具当前仅支持转换 '+PU+' 链接。</div>').join("");
}
function escapeHtml(value){const el=document.createElement("span");el.textContent=value;return el.innerHTML}

convert.addEventListener("click",async()=>{
  if(!input.value.trim()){setStatus("请输入订阅链接。","error");return}
  const {kept,skipped}=splitLinks(input.value);showWarnings(skipped);
  if(!kept.length){setStatus("没有可处理的 "+P+":// 链接。","error");result.classList.remove("visible");return}
  const {masked,fakeToOriginal}=maskIds(kept);
  ...
  try{
    ...
    for(const [index,item] of payload.results.entries()){
      if(item.ok)converted.push(restoreIds(item.converted,fakeToOriginal));
...
export default {
  async fetch(request, env) {
    ...
    const results = await Promise.all(
      body.links.map((link) => convertLink(link, env.SECRET_KEY, env.PROXY_DOMAIN))
    );
    return json({ results });
  },
};

async function convertLink(rawLink, secret, proxyDomain) {
  ...
  if (!match) {
    return { ok: false, error: "无法解析 " + PROTO_UP + " 链接" };
  }

  const [, id, address, port, rawQuery, fragment = ""] = match;
  ...
  const converted =
    `${PROTO}://${id}@${proxyDomain}:${port}?${params.toString()}` +
    (fragment ? `#${fragment}` : "");
